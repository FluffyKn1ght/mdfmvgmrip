...
class VGM:
    ...
    def from_data(file_data: bytes, *args, dump_cmds_to: str | None = None) -> VGM:
        ...
        elif (ym2612_clock & 0x80 == 0x80) & vgm.version >= 0x00000150:
        ...
        if ym2612_clock == 0 and psg_clock == 0:
            raise VGMError(
                "Not a Sega MegaDrive/Genesis .VGM (PSG&YM2612 are not used)"
            )
            ...
            if cmd == 0x66:  # end of VGM data stream
                ...
                if not data_block["type"] in vgm.data.keys():
                    vgm.data[data_block["type"]] = {
                        ...
                        "sections": [{"start": 0, "end": len(data_block["data"])}],
                    }
                else:
                    ...
                    vgm.data[data_block["type"]]["sections"].append(
                        {
                            ...
                            "end": len(vgm.data[data_block["type"]]["data"])
                            + 1
                            + len(data_block["data"]),
                        }
                    )
                ...
                if cmddump:
                    cmddump.write(
                        f"Data block [0x67]: size {len(data_block['data'])}, type {data_block['type']}\n"
                    )
                ...
                if cmddump:
                    cmddump.write(
                        f"DAC stream control command [0x90-0x95, {cmd}] - UNIMPLEMENTED\n"
                    )
                    cmddump.close()
                ...
                if cmddump:
                    cmddump.write(
                        f"YM2612 write from databank and wait {wait_time} samples [0x8n, {cmd}]\n"
                    )
                ...
                if cmddump:
                    cmddump.write(
                        f"YM2612 write [0x52/0x53] - port {port}, reg {register}, val {value} ({bin(value)})\n"
                    )
                i += 2
                vgm.commands.append(
                    {"cmd": "ym_write", "port": port, "reg": register, "val": value}
                )
            elif cmd == 0x4F:  # PSG port 0x06 write
                value: int = data.read(1)[0]
                if cmddump:
                    cmddump.write(
                        f"SEGA PSG (SN76489) stereo write - port 0x06, val {value} ({bin(value)})\n"
                    )
                i += 1
                ...
                if cmddump:
                    cmddump.write(
                        f"SEGA PSG (SN76489) write - val {value} ({bin(value)})\n"
                    )
                i += 1
                ...
                if cmddump:
                    cmddump.write(
                        f"Seek to offset {dbank_offset} in YM2612 PCM databank [0xE0]\n"
                    )
                ...
                if cmddump:
                    ...
                    cmddump.close()
                raise VGMError(
                    f"At byte {i}: Unknown command - wat da hell is {cmd}?!?!?!?"
                )
